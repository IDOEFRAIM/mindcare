...
import { Brain, Linkedin, Instagram, ArrowUpRight } from 'lucide-react';

export default function Footer() {
  const expertise = [
    { label: 'Psychologie', link: 'psychologie' },
    { label: 'Ressources humaines', link: 'rh' },
    { label: 'Formation', link: 'formation' },
    { label: 'Management', link: 'management' },
    { label: 'Santé au travail', link: 'sante' }
  ];

  const cabinet = [
    { label: 'Notre Vision', link: '#' },
    { label: 'Diagnostic', link: 'https://wa.me/+22670813978' },
    { label: 'Le Catalogue', link: '/catalogue' },
    { label: 'Contact', link: 'https://wa.me/+22670813978' }
  ];
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-24 border-b border-white/5 pb-24">
          <div className="max-w-md">
            ...
            <p className="text-[#A5B9CC] text-lg font-light leading-relaxed">
              ...
              <br />Stabiliser, former, performer.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-12 mb-24">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {expertise.map((item) => (
                <li key={item.label}>
                  <Link href={`/expertise/${item.link.toLowerCase()}`} className="text-[#A5B9CC] hover:text-white transition-all flex items-center gap-2 group text-sm">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {cabinet.map((item) => (
                <li key={item.label}>
                  <Link href={item.link} className="text-[#A5B9CC] hover:text-white transition-all text-sm">
                    {item.label}
                  </Link>
                </li>
              ))}
